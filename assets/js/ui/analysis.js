...
import { availability, weightTrend, wattsPerKg, energyView, hydrationSeries, MIN_POINTS } from "../core/body.js";
import { buildBriefing } from "../core/briefing.js";
import { cadenceCoach } from "../core/cadence.js";
import { efficiencyTrend, decouplingTrend, DECOUPLING_STABLE, DECOUPLING_MIN_POINTS } from "../core/efficiency.js";
import { fmt, fmtInt, fmtDate } from "../core/format.js";
import { eftpHistory, eftpHistoryFromWellness, mergeEftpHistories, forecastFtp, dateForTarget } from "../core/ftp-forecast.js";
...
import { overallZoneShares, overallBandsFromIF, distributionShape, LOW_INTENSITY_TARGET } from "../core/zones.js";
...
const LEVEL_COLOR = { green: "var(--z1, #4a9a6e)", yellow: "var(--ss, #e08a3c)", red: "var(--thr, #d94f4f)" };
const RISK_COLOR = { ok: "var(--z1, #4a9a6e)", caution: "var(--ss, #e08a3c)", high: "var(--thr, #d94f4f)" };
...
export const Analysis = {
  ...
  _renderBriefing(ownPlan, today) {
    ...
    const dot = (status) => status === "alert" ? "var(--thr, #d94f4f)"
      : status === "caution" ? "var(--ss, #e08a3c)"
      : status === "nodata" ? "var(--dim2, #6b7280)" : "var(--z1, #4a9a6e)";
  ...
  _renderZones(rides, ownPlan) {
    ...
    const sourceLabel = dist.source === "zoneTimes"
  ...
  _renderPower(ownPlan) {
    ...
    const rows = [
      {
        icon: "🔬", label: "gemessen", value: ac.ftpMeasured,
        meta: ac.ftpMeasuredDate ? `Ramp-Test · ${fmtDate(ac.ftpMeasuredDate)}` : "Ramp-Test (letzter)",
      },
      {
        icon: "〜", label: "geschätzt", value: lastEftp?.eftp ?? null,
        meta: lastEftp ? `eFTP intervals.icu · Stand ${fmtDate(lastEftp.date)} · ≠ Ramp-Test-Wert` : "eFTP intervals.icu",
      },
      {
        icon: "🎯", label: "Ziel", value: ac.ftpGoal,
    ...
    if (history.length >= 3) {
      if (ownPlan && CONFIG.retestDate) {
        ...
        if (t?.reached && t.days === 0) {
          ...
          forecastHtml = `<p class="ftp-forecast-line">${t.reason === "flat"
            ? `Der eFTP-Trend ist aktuell flach — kein belastbarer Zielhorizont für ${ac.ftpGoal} W ableitbar.`
            : `Ziel ${ac.ftpGoal} W liegt beim aktuellen Trend mehr als 12 Monate entfernt.`}</p>`;
        }
      }
    } else {
  ...
  _renderComparison(ownPlan) {
    ...
    const metrics = [
      { label: "Fahrten", p1: p1.length, p2: p2.length, fmt: v => v, unit: "", higherIsBetter: true },
      { label: "Gesamtdistanz", p1: Math.round(sum(p1, "km")), p2: Math.round(sum(p2, "km")), fmt: v => v.toLocaleString("de"), unit: " km", higherIsBetter: true },
      { label: "Ø Kadenz", p1: avg(p1.filter(r => r.kad), "kad"), p2: avg(p2.filter(r => r.kad), "kad"), fmt: v => fmtInt(v), unit: " RPM", higherIsBetter: true },
      { label: "Ø HF", p1: avg(p1.filter(r => r.hf), "hf"), p2: avg(p2.filter(r => r.hf), "hf"), fmt: v => fmtInt(v), unit: " bpm", higherIsBetter: false },
      { label: "Ø NP", p1: avg(p1.filter(r => r.np), "np"), p2: avg(p2.filter(r => r.np), "np"), fmt: v => fmtInt(v), unit: "W", higherIsBetter: true },
      { label: "Peak CTL", p1: maxVal(p1.filter(r => r.ctl), "ctl"), p2: maxVal(p2.filter(r => r.ctl), "ctl"), fmt: v => fmt(v), unit: "", higherIsBetter: true },
      { label: "Ø TSS/Woche",
        p1: sum(p1, "tss") / Math.max(1, [...new Set(p1.map(r => r.week))].length),
        p2: sum(p2, "tss") / Math.max(1, [...new Set(p2.map(r => r.week))].length),
        fmt: v => Math.round(v), unit: "", higherIsBetter: true },
    ];
  ...
  _renderBody(today) {
    ...
    if (!avail.any) { wrap.innerHTML = ""; return; }
    ...
    if (avail.weight) {
      ...
      if (t) {
        ...
        const deltaCol = t.delta30d == null || Math.abs(t.delta30d) < 0.3 ? "var(--dim)"
          : t.delta30d < 0 ? "var(--z1, #4a9a6e)" : "var(--ss, #e08a3c)";
        cards.push(`
          <div class="aerobic-card">
    ...
    if (avail.energy) {
      const e = energyView(Data.wellness, this._allRides);
      if (e) {
        cards.push(`
          <div class="aerobic-card">
            <div class="aerobic-title">Energie</div>
            <div class="aerobic-val">${e.avgIn.toLocaleString("de")}<span class="aerobic-unit">kcal/Tag Ø</span></div>
            <div class="aerobic-sub">Ø Trainingsenergie: ${e.avgTraining.toLocaleString("de")} kcal/Tag (Näherung kJ ≈ kcal) · ${e.n} Tage</div>
            <div class="aerobic-sub">Chronisch niedrige Zufuhr bei hoher Last bremst Regeneration (Energieverfügbarkeit).</div>
          </div>`);
      }
    }
  ...
  _renderConsistency(ownPlan, today) {
    ...
    if (c.adherence) {
      chips.push({ v: `${c.adherence.quote}%`, l: "Plan-Adhärenz", sub: `${c.adherence.done}/${c.adherence.planned} geplante Einheiten absolviert` });
    }
  ...
  _renderPeriodization(ownPlan) {
    ...
    if (!ownPlan) { box.innerHTML = ""; return; }
    ...
    const statusColor = { ok: "var(--z1, #4a9a6e)", teilweise: "var(--ss, #e08a3c)", abweichend: "var(--thr, #d94f4f)" };
    ...
    box.innerHTML = blocksHtml
      ...
      : `<p class="analysis-empty">Noch keine Blockwochen (Sweet Spot / Schwelle / VO₂max) mit Daten.</p>` + recoveryHtml;
  },
};
