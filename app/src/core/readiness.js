...
/** Anzeige-Label je Ampel-Level — einzige Quelle für diesen Wortlaut.
 *  features/hero/ReadinessCard.tsx und core/export-briefing.js importieren
 *  von hier statt eigener Kopien (sonst driften UI-Karte und Claude-
 *  Briefing bei einer Wortlaut-Änderung unbemerkt auseinander). */
export const LEVEL_LABEL = { green: "Bereit", yellow: "Angeschlagen", red: "Erholung nötig" };

/** Hinweistext für die Schlafqualitäts-Metrik (sleepScore) — Athlet 1 und
 *  Athlet 2 liefern diesen Wert über unterschiedliche Ökosysteme (Apple
 *  Health bzw. Amazfit, s. `dataSources` in config.ts), die jeweils ihren
 *  eigenen, nicht offengelegten Score-Algorithmus verwenden. Die Ampel
 *  selbst ist davon NICHT betroffen — jeder Athlet wird ausschließlich
 *  gegen seine eigene 42-Tage-Baseline verglichen (nie athletenübergreifend,
 *  s. assessReadiness oben) — dieser Text ist reine Lese-Vorsicht für
 *  Menschen (und Claude im Trainer-Briefing), die die rohen Zahlen zweier
 *  Athleten nebeneinander sehen und für vergleichbar halten könnten. Anders
 *  als HRV (ms) oder Ruhepuls (bpm) ist "Schlafqualität" keine physikalische
 *  Einheit, sondern ein herstellereigener Index — deshalb nur bei dieser
 *  einen Metrik nötig, nicht bei den übrigen dreien. */
export const SLEEP_SCORE_DEVICE_NOTE =
  "Schlafqualität ist geräteabhängig (App-eigener Score) — nicht athletenübergreifend vergleichbar.";
