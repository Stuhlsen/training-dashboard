...
➡️ **Phase 3, Umsetzung — Nach-Drop-Feedback (Schritt 5) [OP/SO]:** Die Rechen-/Regelschicht (Schritt 4: `core/projection.js`, `core/conflicts.js`, `core/plan-config.js`, angebunden über `state/plan-cards.js::getState().projection`/`.conflicts`) ist fertig und liefert die Daten. Schritt 5 rendert sie nur noch im Planungstab (`ui/planned.js`): Delta-Zeile bei einem Event im Horizont („TSB am Eventtag: +12 → +6"), Konflikt-Badges an den betroffenen Karten/Tagen (gold = `info`, rot = `warning`, Text aus `conflict.message`), Push-Warnung bei Verschieben einer bereits gepushten Karte (`pushed_external_id`, beide Eingabearten zugleich — s. `docs/offene-punkte.md`). Kein neuer Rechencode nötig — nur Anzeige. Drag-Live-Färbung (K2) bleibt ein Polish-Schritt danach.
