...
export function getTitle(p) { return p?.type === "title" ? p.title?.map(t => t.plain_text).join("") || "" : ""; }
...
export function getNum(p) { return p?.type === "number" ? p.number : null; }
export function getCheckbox(p) { return p?.type === "checkbox" ? p.checkbox || false : false; }
export function getDate(p) { return p?.type === "date" && p.date ? p.date.start || null : null; }
export function getRichText(p) { return p?.type === "rich_text" ? p.rich_text?.map(t => t.plain_text).join("") || "" : ""; }
...
export async function queryNotionPlan1() {
  ...
  while (hasMore) {
    ...
    const res = await fetch(`https://api.notion.com/v1/databases/${ENV.DB_ID}/query`, {
      method: "POST",
      headers: { Authorization: `Bearer ${ENV.NOTION_KEY}`, "Notion-Version": "2022-06-28", "Content-Type": "application/json" },
